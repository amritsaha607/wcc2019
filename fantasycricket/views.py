...
CRICKET_API = 'cFkEEvLr6Yh9pulkk9BzhWch15C3'
...
teamsArr = ["india","pakistan","west indies","bangladesh","australia","south africa","new zealand","england","sri lanka","afghanistan","tba"]
############################ FIND SCORES #############
...
def FAQ(request):
    ...
    return render(request, 'FAQ.html', {'log': log})
...
def AboutUs(request):
    ...
    if 'user_id' in request.session:
        ...
        user = db.collection('Users').document(request.session['user_id']).get()
        username = user.get('Username')
    return render(request, 'AboutUs.html', {'log': log})
...
def matches(request):
    ...
    for x in json_data_new['matches']:
        if (x['type']=='ODI' and teamsArr.count(x['team-1'].lower())>0):
            print('x = ' + str(x))
            ...
            x['img1'] = "/static/image/countryflags/" + str(tags.filtercut(x,'team-1').lower()) + ".png"
            x['img2'] = "/static/image/countryflags/" + str(tags.filtercut(x,'team-2').lower()) + ".png"
            #x['img2'] = bucket(name='fantasycricket-af35d.appspot.com/mycricket/countryflags/'+str(tags.filtercut(x,'team-2').lower())+'.png')
                
            print(x['img1'])
            print(x['img2'])
            newMatches.append(x)
    return render(request, 'matches.html', {'matches': newMatches, 'log': log})
    #return redirect('fantasycricket:login')
...
def contests(request):
    if 'user_id' in request.session:
        ...
        if 'already_enrolled' in request.session:
            already_enrolled = request.session['already_enrolled']
            del request.session['already_enrolled']
        toShow = []
        ...
        return render(request, 'contests.html', {'contests': toShow, 'already_enrolled': already_enrolled})
    return redirect('fantasycricket:login')
    ...
    if 'user_id' in request.session:
        ...
        if request.method=='POST':
            if 'choice_contest_id' in request.POST:
                already_enrolled = db.collection('Matches').document(str(request.session['choice_match_id'])).collection('Contests').document(request.POST['choice_contest_id']).collection('Participants').document(username).get().to_dict()
                if already_enrolled:
                    print('\n\nohh already enrolled\n\n')
                    request.session['already_enrolled'] = "You are already a participant in this contest"
                    return redirect('fantasycricket:contests')
...
def liveScore(request):
    if 'user_id' in request.session:
        user = db.collection('Users').document(request.session['user_id'])
        username = user.get().get('Username')
        batting1, batting2, bowling1, bowling2, fielding1, fielding2 = None, None, None, None, None, None
        if request.method=='POST':
            print(request.POST)
            request.session['choice_match_id'] = request.POST['choice_match_id']
            return redirect('fantasycricket:liveScore')
        if request.method=='GET':
            match = db.collection('Matches').document(str(request.session['choice_match_id']))
            
            ## For finished match , load from the database
            #print(match.get().to_dict())
            if match.get().to_dict():
                newData = match.get().to_dict()['Team']
                print(newData[0])
                batting1, batting2, bowling1, bowling2 = {'title':'', 'scores':[]}, {'title':'', 'scores':[]}, {'title':'', 'scores':[]}, {'title':'', 'scores':[]}
                batting1['title'] = str(newData[0]['team'])
                bowling2['title'] = str(newData[0]['team'])
                for x in newData:
                    if 'team' in x:
                        if not x['team']==newData[0]['team']:
                            if x['name']=='Extras':
                                print('\n\nyo mil gye\n\n')
                                batting2['title'] = str(x['team'])
                                bowling1['title'] = str(x['team'])
                                print(batting2['title'])
                                break
                                
                for x in newData:
                    if 'team' in x:
                        ## Batting Data
                        if x['team']==batting1['title']:
                            newObject = {
                                'batsman': x['name'],
                                'R': x['runs'],
                                'B': x['bowls_conceded'],
                                'dismissal': x['dismissal'],
                                'dismissal-info': x['dismissal-info'],
                                '4s': x['fours'],
                                '6s': x['sixes'],
                                'SR': x['strike_rate'],
                            }
                            batting1['scores'].append(newObject)
                        elif x['team']==batting2['title']:
                            newObject = {
                                'batsman': x['name'],
                                'R': x['runs'],
                                'B': x['bowls_conceded'],
                            ...
                            batting2['scores'].append(newObject)

                        ## Bowling Data
                        if x['team']==bowling1['title']:
                            newObject = {
                                'bowler': x['name'],
                                'O': x['overs_bowled'],
                                'R': x['runs_conceded'],
                            ...
                            bowling1['scores'].append(newObject)
                        elif x['team']==bowling2['title']:
                            newObject = {
                                'bowler': x['name'],
                                'O': x['overs_bowled'],
                                'R': x['runs_conceded'],
                            ...
                            bowling2['scores'].append(newObject)

                            
                            
                            
                           
            ## IF MATCH IS FINISHED AND THE DATA OF THE MATCH IS NOT IN THE DATABASE , THEN SAVE IT TO DATABASE
            else:
                scoreData = urllib.request.urlopen('https://cricapi.com/api/fantasySummary?apikey=' + str(CRICKET_API) + '&unique_id=' + str(request.session['choice_match_id']))
                json_data_new = json.loads(scoreData.read().decode(scoreData.info().get_param('charset') or 'utf-8'))
                batting1 = json_data_new['data']['batting'][0]#['scores']
                print("\n\nbatting1['title'] : " + str(batting1['title']))
                if len(json_data_new['data']['batting'])>1:
                    batting2 = json_data_new['data']['batting'][1]#['scores']
                bowling1 = json_data_new['data']['bowling'][0]#['scores']
                if len(json_data_new['data']['bowling'])>1:
                    bowling2 = json_data_new['data']['bowling'][1]#['scores']
                fielding1 = json_data_new['data']['fielding'][0]
                if len(json_data_new['data']['bowling'])>1:
                    fielding2 = json_data_new['data']['fielding'][1]
                print('\n\nfielding1 : ' + str(fielding1)+'\n\n\n')
                
                # not in database
                if ((not match.get().to_dict()) and ('winner_team' in json_data_new['data'])):
                    newArr=[]
                    ...
                    for x in batting1['scores']:
                        dismiss1 = "not out"
                        if 'dismissal' in x:
                            dismiss1 = x['dismissal']
                        print('\n\ndismissak info : ' + str(x['dismissal-info']))
                        t = batting1['title']
                        t = t[:len(t)-8]
                        if x['batsman']=='Extras':
                            newObj = {
                        ...
                        #print('\n\nx:'+str(x)+'\n\n')
                        newObj['batting_points'] = BattingScore(x)
                        newObj['points'] = newObj['batting_points']
                        if bowling2:
                            for y in bowling2['scores']:
                                if y['pid']==newObj['pid']:
                                    newObj['economy'] = y['Econ']
                                    newObj['maidens'] = y['M']
                                    newObj['overs_bowled'] = y['O']
                                    newObj['runs_conceded'] = y['R']
                                    newObj['wicket_taken'] = y['W']

                                    newObj['bowling_points'] = BowlingScore(y)
                                    newObj['points'] += newObj['bowling_points']

                                    break

                        if fielding2:
                            for y in fielding2['scores']:
                                if y['pid']==newObj['pid']:
                                    newObj['bowled'] = y['bowled']
                                    newObj['catchball'] = y['catch']
                                    newObj['lbw'] = y['lbw']
                                    newObj['runout'] = y['runout']
                                    newObj['stumped'] = y['stumped']

                                    newObj['fielding_points'] = FieldingScore(y)
                                    newObj['points'] += newObj['fielding_points']

                                    break
                            newArr.append(newObj)
                        
                        
                    for x in batting2['scores']:
                        dismiss2 = "not out"
                        if 'dismissal' in x:
                            dismiss2 = x['dismissal']
                    ...
                    ## Make a dictionary to save in the database
                    dictToAdd = {
                        'Team': newArr
                    }
                    match.set(dictToAdd)
        ...
        return render(request, 'liveScore.html', {'bowling1': bowling1, 'bowling2': bowling2, 'batting1': batting1, 'batting2': batting2})
    return redirect('fantasycricket:login')
